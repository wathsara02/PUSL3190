...
Use Python 3.10 or newer. Python 3.12 is the tested path for this repository.

Dependencies are pinned in `requirements.txt`. It installs the tested CPU
PyTorch build and matching library versions for this project.

Important:
- Do not upload or commit your local `.venv/` folder.
- Anyone who downloads the repo should create their own virtual environment.
- If `python` resolves to the Windows Store shim, install a real Python release first.
...
If `python -m venv .venv` fails on Windows, install Python 3.12 from
python.org, reopen the terminal, and run the same commands again.

## Verify The Install

Run the lightweight setup check:
...
If a script says packages like `torch` are missing even though you installed
them before, you are almost certainly using the wrong interpreter or a stale
virtual environment. Recreate `.venv` and reinstall from `requirements.txt`.
...
# Verify interpreter and core packages
python scripts/check_setup.py
