...
class MAPPOTrainer:
    ...
    def collect_episode(self, env) -> Tuple[List[dict], dict]:
        ...
        while active_envs:
            if is_vector:
                # Combined round trip: agent_selection + observe (was 2 separate IPC calls).
                selection_obs = env.observe_with_selection(active_envs)
                agent_names = [r[0] for r in selection_obs]
                obs_list    = [r[1] for r in selection_obs]
            else:
            ...
            if is_vector:
                # Combined round trip: step + cumulative_rewards + terminations + infos + state.
                # Was 4 separate IPC calls (step, get_cumulative_rewards, get_terminations, get_infos).
                collect_results = env.step_collect(final_actions, active_envs)
                cumulative_rewards_list = [r[0] for r in collect_results]
                terminations_list       = [r[1] for r in collect_results]
                infos_list              = [r[2] for r in collect_results]

            else:
                env.step(int(final_actions[0]))
                cumulative_rewards_list = [env._cumulative_rewards]
                terminations_list = [env.terminations]
                infos_list = [env.infos]
