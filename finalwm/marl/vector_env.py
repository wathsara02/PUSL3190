...
def worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        try:
            ...
            elif cmd == 'observe_with_selection':
                # Combines agent_selection + observe into one round trip.
                agent = env.agent_selection
                obs = env.observe(agent)
                remote.send((agent, obs))
            elif cmd == 'step_collect':
                # Combines step + cumulative_rewards + terminations + infos into one round trip.
                env.step(data)
                remote.send((
...
class CloudVectorEnv:
    ...
    def observe_with_selection(self, env_indices: list = None):
        """Single round trip returning (agent_name, obs_dict) per env."""
        if env_indices is None: env_indices = list(range(self.num_envs))
        return self._send_to_active('observe_with_selection', [None]*len(env_indices), env_indices)

    def step_collect(self, actions: list, env_indices: list = None):
        """Single round trip: step then return (cumulative_rewards, terminations, infos, state)."""
        if env_indices is None: env_indices = list(range(self.num_envs))
        return self._send_to_active('step_collect', actions, env_indices)
