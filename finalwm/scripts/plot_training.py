...
Can be called:
  ...
  - Standalone from a CSV:
      python scripts/plot_training.py --csv runs/<exp>/training_summary.csv
  - Standalone for a full run directory (generates all charts including extras):
      python scripts/plot_training.py --run-dir runs/local_5600g
"""
...
def _collect_checkpoint_rows(evals_dir: Path) -> pd.DataFrame:
    """Walk every eval_ep_* sub-folder and pull the final-progress row."""
    rows = []
    ep_pattern = re.compile(r"eval_ep_(\d+)$")
    for folder in sorted(evals_dir.iterdir()):
        m = ep_pattern.match(folder.name)
        if not m:
            continue
        training_ep = int(m.group(1))
        csv = folder / "evaluation_summary.csv"
        if not csv.exists():
            continue
        try:
            df = pd.read_csv(csv)
        ...
        if df.empty:
            continue
        row = df.sort_values("progress_pct").iloc[-1]
        agent_wr    = float(row["agent_win_rate"])
        baseline_wr = float(row["baseline_win_rate"])
        draw_wr     = float(row.get("draw_rate", 0.0))
        total_decisive = agent_wr + baseline_wr
        decisive_wr = (agent_wr / total_decisive * 100.0) if total_decisive > 0 else 50.0
        rows.append({
            "training_ep": training_ep,
            "agent_win_rate": agent_wr,
            "baseline_win_rate": baseline_wr,
            "draw_rate": draw_wr,
            "decisive_win_rate": decisive_wr,
        })
    return pd.DataFrame(rows).sort_values("training_ep").reset_index(drop=True)


def _plot_eval_progression(run_dir: Path, out_dir: Path):
    evals_dir = run_dir / "baseline_evals"
    if not evals_dir.exists():
        print("[PLOT] No baseline_evals directory — skipping eval_progression.")
        return
    df = _collect_checkpoint_rows(evals_dir)
    if df.empty:
        print("[PLOT] No checkpoint data found — skipping eval_progression.")
        return

    x         = df["training_ep"].values
    decisive  = df["decisive_win_rate"].values
    agent_raw = df["agent_win_rate"].values
    base_raw  = df["baseline_win_rate"].values
    draw_raw  = df["draw_rate"].values
    sw = max(3, len(df) // 8)

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle("Evaluation Performance over Training", fontsize=14, fontweight="bold")

    ax = axes[0]
    ax.plot(x, decisive, color="steelblue", alpha=0.3, linewidth=1)
    if len(df) >= sw:
        ax.plot(x[sw - 1:], _smooth(decisive, sw),
                color="steelblue", linewidth=2.5, label="Decisive win rate")
    ax.axhline(50, color="gray",   linestyle="--", linewidth=1, alpha=0.7, label="50% (chance)")
    ax.axhline(33, color="salmon", linestyle=":",  linewidth=1, alpha=0.7, label="33% (random)")
    ax.set_title("Decisive Win Rate vs Baseline\n(wins / (wins + losses), draws excluded)")
    ax.set_xlabel("Training episodes")
    ax.set_ylabel("Decisive win rate (%)")
    ax.set_ylim(0, 100)
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

    ax = axes[1]
    ax.plot(x, agent_raw, color="steelblue", alpha=0.25, linewidth=1)
    ax.plot(x, base_raw,  color="tomato",    alpha=0.25, linewidth=1)
    ax.plot(x, draw_raw,  color="seagreen",  alpha=0.25, linewidth=1)
    if len(df) >= sw:
        sm = x[sw - 1:]
        ax.plot(sm, _smooth(agent_raw, sw), color="steelblue", linewidth=2.5, label="Agent wins")
        ax.plot(sm, _smooth(base_raw,  sw), color="tomato",    linewidth=2.5, label="Baseline wins")
        ax.plot(sm, _smooth(draw_raw,  sw), color="seagreen",  linewidth=2.0, label="Draws")
    ax.axhline(50, color="gray", linestyle="--", linewidth=1, alpha=0.6)
    ax.set_title("Raw Outcome Rates vs Baseline")
    ax.set_xlabel("Training episodes")
    ax.set_ylabel("Outcome rate (%)")
    ax.set_ylim(0, 100)
    ax.legend(fontsize=9)
    ...
    _save_figure(fig, out_dir / "eval_progression.png")
...
def _plot_score_distribution(run_dir: Path, out_dir: Path):
    csv_path = run_dir / "evaluation_match_traces.csv"
    if not csv_path.exists():
        csv_path = run_dir / "evaluation_result.csv"
    if not csv_path.exists():
        print("[PLOT] No evaluation_match_traces.csv — skipping score_distribution.")
        return

    df = pd.read_csv(csv_path)
    if "final_score" not in df.columns or "winner_team" not in df.columns:
        print("[PLOT] Missing final_score/winner_team columns — skipping score_distribution.")
        return

    parsed = df["final_score"].apply(_parse_score)
    df = df.copy()
    df["score_a"] = parsed.apply(lambda x: x[0] if x else np.nan)
    df["score_b"] = parsed.apply(lambda x: x[1] if x else np.nan)
    df = df.dropna(subset=["score_a", "score_b"])
    df["score_a"] = df["score_a"].astype(int)
    df["score_b"] = df["score_b"].astype(int)

    total         = len(df)
    agent_wins    = df[df["winner_team"] == 0]
    baseline_wins = df[df["winner_team"] == 1]
    draws         = df[df["winner_team"] == -1]

    labels          = ["5-3", "6-2", "7-1", "8-0"]
    pairs_agent     = [(5,3),(6,2),(7,1),(8,0)]
    pairs_baseline  = [(3,5),(2,6),(1,7),(0,8)]
    agent_counts    = [len(agent_wins[(agent_wins["score_a"]==sa)&(agent_wins["score_b"]==sb)])
                       for sa, sb in pairs_agent]
    baseline_counts = [len(baseline_wins[(baseline_wins["score_a"]==la)&(baseline_wins["score_b"]==lb)])
                       for la, lb in pairs_baseline]

    x = np.arange(len(labels))
    w = 0.35

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle("Score Distribution — Evaluation Games", fontsize=14, fontweight="bold")

    ax = axes[0]
    bars_a = ax.bar(x - w/2, agent_counts,    w, label="Agent wins",    color="steelblue", alpha=0.85)
    bars_b = ax.bar(x + w/2, baseline_counts, w, label="Baseline wins", color="tomato",    alpha=0.85)
    ax.bar(len(labels), len(draws), w * 1.5, label="Draws (4-4)", color="seagreen", alpha=0.7)
    ...
    ax.set_xticklabels(labels + ["4-4\n(Draw)"])
    ax.set_xlabel("Score (Team 0 tricks : Team 1 tricks)")
    ax.set_ylabel("Number of games")
    ax.set_title(f"Score Breakdown (n={total:,} games)")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, axis="y")

    ax = axes[1]
    agent_margins    = (agent_wins["score_a"]    - agent_wins["score_b"]).values
    baseline_margins = (baseline_wins["score_b"] - baseline_wins["score_a"]).values
    bins = np.arange(0.5, 9.5, 1)
    ax.hist(agent_margins,    bins=bins, alpha=0.7, color="steelblue",
            label=f"Agent wins (n={len(agent_wins):,})",    density=True)
    ax.hist(baseline_margins, bins=bins, alpha=0.7, color="tomato",
            label=f"Baseline wins (n={len(baseline_wins):,})", density=True)
    ax.set_xticks(range(1, 9))
    ax.set_xlabel("Trick margin (winning tricks − losing tricks)")
    ax.set_ylabel("Density")
    ax.set_title("Win Margin Distribution")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, axis="y")

    _save_figure(fig, out_dir / "score_distribution.png")


def _plot_declarer_win_rate(run_dir: Path, out_dir: Path):
    csv_path = run_dir / "training_summary.csv"
    if not csv_path.exists():
        print("[PLOT] training_summary.csv not found — skipping declarer_win_rate.")
        return

    df = pd.read_csv(csv_path)
    if "declarer_team_win_events" not in df.columns:
        print("[PLOT] Declarer event columns missing — skipping declarer_win_rate.")
        return

    wins      = df["declarer_team_win_events"].fillna(0).values.astype(float)
    losses    = df["declarer_team_loss_events"].fillna(0).values.astype(float)
    total     = wins + losses
    rate      = np.where(total > 0, wins / total * 100.0, np.nan)
    episodes  = df["episodes_completed"].values
    block_eps = df["block_episodes"].values.astype(float)
    win_per_ep  = np.where(block_eps > 0, wins   / block_eps, 0.0)
    loss_per_ep = np.where(block_eps > 0, losses / block_eps, 0.0)
    sw = max(3, len(df) // 10)

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle("Trump Declaration Quality over Training", fontsize=14, fontweight="bold")

    ax = axes[0]
    valid = ~np.isnan(rate)
    ax.plot(episodes[valid], rate[valid], color="goldenrod", alpha=0.3, linewidth=1)
    if valid.sum() >= sw:
        ev = episodes[valid]; rv = rate[valid]
        ax.plot(ev[sw - 1:], _smooth(rv, sw), color="goldenrod", linewidth=2.5, label="Declarer win rate")
    ax.axhline(50, color="gray", linestyle="--", linewidth=1, alpha=0.7, label="50% (neutral)")
    ax.set_title("Declarer Team Win Rate\n(wins / (wins + losses) per training block)")
    ax.set_xlabel("Training episodes")
    ax.set_ylabel("Declarer win rate (%)")
    ax.set_ylim(0, 100)
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

    ax = axes[1]
    ax.plot(episodes, win_per_ep,  color="steelblue", alpha=0.25, linewidth=1)
    ax.plot(episodes, loss_per_ep, color="tomato",    alpha=0.25, linewidth=1)
    if len(df) >= sw:
        sm = episodes[sw - 1:]
        ax.plot(sm, _smooth(win_per_ep,  sw), color="steelblue", linewidth=2.5, label="Declarer wins / ep")
        ax.plot(sm, _smooth(loss_per_ep, sw), color="tomato",    linewidth=2.5, label="Declarer losses / ep")
    ax.set_title("Declarer Team Outcomes per Episode")
    ax.set_xlabel("Training episodes")
    ax.set_ylabel("Events / episode")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

    _save_figure(fig, out_dir / "declarer_win_rate.png")


def plot_extra(run_dir: str | Path, out_dir: str | Path | None = None):
    """Generate all three supplementary charts for a completed training run."""
    run_dir = Path(run_dir)
    out_dir = Path(out_dir) if out_dir is not None else run_dir
    out_dir.mkdir(parents=True, exist_ok=True)
    _plot_eval_progression(run_dir, out_dir)
    _plot_score_distribution(run_dir, out_dir)
    _plot_declarer_win_rate(run_dir, out_dir)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot Omi training progress from CSV.")
    parser.add_argument("--csv",      type=str, default=None, help="Path to training_summary.csv")
    parser.add_argument("--run-dir",  type=str, default=None, help="Run directory — generates all charts")
    parser.add_argument("--out",      type=str, default=None, help="Output directory (default: CSV/run directory)")
    ...
    if args.run_dir:
        ...
        plot_training(run_dir / "training_summary.csv", out_dir)
        agg_csv = run_dir / "baseline_evals" / "baseline_eval_results.csv"
        if agg_csv.exists():
            plot_baseline_evals(agg_csv, out_dir)
        plot_extra(run_dir, out_dir)
    elif args.csv:
        plot_training(args.csv, args.out)
        if args.eval_csv:
            plot_baseline_evals(args.eval_csv, args.out)
    else:
        parser.error("Provide either --csv or --run-dir.")
