...
# CPU thread tuning: limit PyTorch threads so worker processes don't fight for cores.
# With 10 spawn workers + main process on a 12-thread CPU, default "use all cores"
# causes severe contention. 2 threads per process lets the OS schedule workers cleanly.
...
class TrainingProgress:
    ...
    def _terminal_width(self) -> int:
        return max(60, shutil.get_terminal_size(fallback=(120, 20)).columns)
    ...
    def update(self, episodes_done: int, totals: dict, losses: Optional[dict] = None) -> None:
        ...
        segments = [
            ...
            f"A {team_a_rate:4.1f}%",
        ]
        if not tiny and losses:
            segments.append(f"pi {losses.get('policy_loss', 0.0):.3f}")
        if not compact and losses:
            segments.append(f"v {losses.get('value_loss', 0.0):.3f}")
            segments.append(f"H {losses.get('entropy', 0.0):.3f}")

        line = "\r" + " | ".join(segments)
...
def main():
    ...
    block_size = max(1, int(cfg["training"].get("log_interval", 10000)))
